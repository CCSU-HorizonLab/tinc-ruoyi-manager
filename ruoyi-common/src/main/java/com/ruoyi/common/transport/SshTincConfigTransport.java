...
public class SshTincConfigTransport implements TincConfigTransport {
    ...
    private static final Pattern REMOTE_PATH = Pattern.compile(
            "/etc/tinc/[A-Za-z0-9_]{1,64}/(?:tinc\\.conf|tinc-(?:up|down)(?:\\.bat)?|rsa_key\\.priv|hosts/[A-Za-z0-9_]{1,64})");
    ...
    public void pushFile(String targetHost, String remotePath, String content, boolean executable) {
        ...
        try {
            ...
            ByteArrayInputStream bis = new ByteArrayInputStream(safeContent.getBytes(StandardCharsets.UTF_8));
            try {
                sftp.put(bis, temporaryPath, ChannelSftp.OVERWRITE);
                if (executable) {
                    sftp.chmod(0755, temporaryPath);
                }
                // SFTP 同目录 rename 在网关文件系统内完成替换，避免 tincd 读到半写入的 hosts 文件。
                sftp.rename(temporaryPath, remotePath);
            } finally {
                bis.close();
                try {
                    sftp.rm(temporaryPath);
                } catch (SftpException ignored) {
                    // rename 成功后临时路径已不存在；失败时尽力清理。
                }
            }
            ...
            log.info("SSH 推送成功 → {}", remotePath);
        } catch (Exception e) {
            log.error("SSH 推送失败 → {}，错误类型={}", remotePath, e.getClass().getSimpleName());
            throw new RuntimeException("无法推送配置文件到 Tinc 网关");
        } finally {
    ...
    public String readFile(String targetHost, String remotePath) {
        ...
        try {
            ...
            if (output.size() > 1024 * 1024) {
                throw new IllegalStateException("Tinc configuration file exceeds limit");
            }
            return new String(output.toByteArray(), StandardCharsets.UTF_8);
        } catch (SftpException e) {
            if (e.id == ChannelSftp.SSH_FX_NO_SUCH_FILE) return null;
            throw new RuntimeException("无法读取 Tinc 网关配置");
        } catch (Exception e) {
            throw new RuntimeException("无法读取 Tinc 网关配置");
        } finally {
            if (sftp != null && sftp.isConnected()) sftp.disconnect();
    ...
    public boolean isTincInstalled(String targetHost) {
        ...
        log.info("Tinc 安装检测 → {}", installed ? "已安装" : "未安装");
        return installed;
    }

    @Override
    public String installTinc(String targetHost) {
        log.info("开始自动安装 Tinc...");
        ...
        log.info("Tinc 安装完成，exit={}", installResult.exitStatus);

        if (installResult.exitStatus != 0) {
            throw new RuntimeException("Tinc 安装失败，退出码: " + installResult.exitStatus);
        }

        // 3. 创建 /etc/tinc 基础目录结构
        execCommandWithResult(targetHost, "mkdir -p /etc/tinc && echo 'OK'");
        log.info("Tinc 环境初始化完成");
    ...
    private void execCommand(String targetHost, String command) {
        ...
        if (result.exitStatus != 0) {
            log.error("远程命令返回非零退出码: exit={}, cmd={}", result.exitStatus, command);
            throw new RuntimeException("Tinc 网关远程命令执行失败，退出码: " + result.exitStatus);
        }
    }
    ...
    private CommandResult execCommandWithResult(String targetHost, String command) {
        ...
        try {
            ...
            log.info("SSH 命令执行完毕，exit={}", exitStatus);
            return new CommandResult(exitStatus, output.toString().trim());
        } catch (Exception e) {
            log.error("SSH 命令执行失败: {}，错误类型={}", command, e.getClass().getSimpleName());
            throw new RuntimeException("无法在 Tinc 网关执行命令");
        } finally {
    ...
    public void deleteNetwork(String targetHost, String netName) {
        ...
        log.info("SSH 远程网络物理删除成功 [{}]", netName);
    }
    ...
    public void deleteNode(String targetHost, String netName, String nodeName) {
        ...
        log.info("SSH 远程删除节点 [{}] 属于网络 [{}]", nodeName, netName);
        // 1. 物理删除节点公钥 hosts 文件
        ...
        log.info("SSH 远程节点删除并重载成功 [{}]", nodeName);
    }
    ...
    private static void requireRemotePath(String value) {
        if (value == null || !REMOTE_PATH.matcher(value).matches()) {
            throw new IllegalArgumentException("Tinc remote path is not allow-listed");
        }
    }
}
